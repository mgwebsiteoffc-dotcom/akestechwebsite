...
ob_start();
?>
<section class="ak-pagehero ak-pagehero--dark"><div class="ak-container ak-pagehero__inner">
...
<main class="ak-section"><div class="ak-container">
<div class="grid lg:grid-cols-5 gap-7 items-start">
    ...
    <aside class="lg:col-span-2 space-y-5 lg:sticky lg:top-24" aria-live="polite">
        ...
        <div class="grid grid-cols-2 gap-4">
            <?php foreach ([['actualRoas','Actual ROAS'],['breakEvenRoas','Break-even ROAS'],['targetRoas','Target ROAS'],['maxCpa','Max CPA at target']] as $result): ?><div class="ak-card ak-card--flat"><p class="text-xs text-gray-500 leading-tight"><?= $result[1] ?></p><p id="<?= $result[0] ?>" class="text-2xl font-extrabold text-gray-900 mt-2">—</p></div><?php endforeach; ?>
        </div>
        <div class="ak-card ak-card--flat"><div class="flex justify-between text-sm mb-2"><span class="text-gray-600">Contribution margin</span><strong id="marginText">0%</strong></div><div class="h-2.5 bg-gray-100 rounded-full overflow-hidden"><div id="marginBar" class="h-full bg-orange-500 rounded-full" style="width:0%"></div></div><div class="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-gray-100 text-sm"><div><span class="block text-gray-500">Orders</span><strong id="orders">0</strong></div><div><span class="block text-gray-500">Profit margin</span><strong id="profitMargin">0%</strong></div></div></div>
    </aside>
</div>

<section class="mt-16 grid lg:grid-cols-2 gap-7"><div class="ak-card ak-card--flat"><h2 class="ak-h3">How to read your result</h2><div class="space-y-4 text-gray-600 leading-relaxed"><p><strong class="text-gray-900">Actual ROAS</strong> reports past performance. It does not show whether that performance was profitable.</p><p><strong class="text-gray-900">Break-even ROAS</strong> is your minimum before overhead. A campaign below it loses contribution profit on the first order.</p><p><strong class="text-gray-900">Target ROAS</strong> includes your chosen net margin, giving your media team a more useful operating target.</p></div></div>
<div class="bg-orange-50 border border-orange-100 rounded-2xl p-7"><h2 class="ak-h3">ROAS formulas</h2><div class="space-y-3 text-sm text-gray-700"><p><code class="bg-white px-2 py-1 rounded">ROAS = ad revenue ÷ ad spend</code></p><p><code class="bg-white px-2 py-1 rounded">Break-even ROAS = 1 ÷ contribution margin rate</code></p><p><code class="bg-white px-2 py-1 rounded">Max CPA = contribution/order − target profit/order</code></p></div><p class="mt-4 text-xs text-gray-500">For business decisions, also account for salaries, tools, warehousing, taxes, returns, and repeat purchase value.</p></div></section>

<section class="ak-container ak-narrow"><h2 class="ak-h3 ak-center">ROAS calculator FAQs</h2><div class="space-y-4"><?php foreach($faqs as $faq): ?><details class="ak-details"><summary class="ak-details__q"><?= clean($faq['question']) ?></summary><p class="ak-details__a"><?= clean($faq['answer']) ?></p></details><?php endforeach; ?></div></section>
</div></main>

<section class="ak-section ak-section--tight ak-darksec"><div class="ak-container ak-narrow"><h2 class="ak-h2 ak-onDark">Need to improve the number?</h2><p class="text-primary-100 mt-3 mb-7">Our performance team connects media efficiency with store conversion and contribution margin.</p><a href="<?= url('services/performance-marketing') ?>" class="ak-btn ak-btn--light">Explore performance marketing</a></div></section>
